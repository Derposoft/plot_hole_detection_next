...
class DataGenerator(keras.utils.Sequence):
    ...
    def __init__(
        ...
        if mode not in ("point", "pair", "list"):
            raise ValueError(
                f"{mode} is not a valid mode type."
        ...
        if mode == "pair":
            data_pack.relation = self._reorganize_pair_wise(
                data_pack.relation, num_dup=num_dup, num_neg=num_neg
            )
    ...
    def reset_index(self):
        ...
        if self._mode == "point":
            ...
            raise NotImplementedError(f"{self._mode} data generator not implemented.")
        else:
            raise ValueError(
                f"{self._mode} is not a valid mode type"
                f"Must be one of `point`, `pair` or `list`."
            )
    ...
    def _reorganize_pair_wise(
        cls, relation: pd.DataFrame, num_dup: int = 1, num_neg: int = 1
    ):
        """Re-organize the data pack as pair-wise format."""
        pairs = []
        groups = relation.sort_values("label", ascending=False).groupby("id_left")
