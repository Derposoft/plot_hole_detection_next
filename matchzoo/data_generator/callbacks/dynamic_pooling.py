...
def _dynamic_pooling_index(
    length_left: np.array,
    ...
    def _dpool_index(
        ...
        one_idx_left = [int(i / stride_left) for i in range(fixed_length_left)]
        one_idx_right = [int(i / stride_right) for i in range(fixed_length_right)]
        mesh1, mesh2 = np.meshgrid(one_idx_left, one_idx_right)
        index_one = np.transpose(np.stack([mesh1, mesh2]), (2, 1, 0))
        return index_one
    ...
    cur_fixed_length_left = (
        int(fixed_length_left // compress_ratio_left) + dpool_bias_left
    )
    cur_fixed_length_right = (
        int(fixed_length_right // compress_ratio_right) + dpool_bias_right
    )
