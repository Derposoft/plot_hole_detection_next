...
def _convert_to_list_index(index: typing.Union[int, slice, np.array], length: int):
...
class DataPack(object):
    ...
    def __init__(self, relation: pd.DataFrame, left: pd.DataFrame, right: pd.DataFrame):
    ...
    def unpack(
        self,
    ) -> typing.Tuple[typing.Dict[str, np.array], typing.Optional[np.array]]:
        """
    ...
    def __getitem__(self, index: typing.Union[int, slice, np.array]) -> "DataPack":
        ...
        return DataPack(left=left.copy(), right=right.copy(), relation=relation.copy())
    ...
    def save(self, dirpath: typing.Union[str, Path]):
        ...
        if data_file_path.exists():
            raise FileExistsError("%s already exist, fail to save" % data_file_path)
    ...
    def _optional_inplace(func):
        ...
        """
        doc = (
            ":param inplace: `True` to modify inplace, `False` to return "
    ...
    def append_text_length(self, verbose=1):
        ...
        """
        self.apply_on_text(
            len, rename=("length_left", "length_right"), inplace=True, verbose=verbose
        )
...
'text_right', 'length_right', 'extra_right', 'label']
    ...
    class FrameView(object):
        ...
        def __getitem__(
            self, index: typing.Union[int, slice, np.array]
        ) -> pd.DataFrame:
            """Slicer."""
            dp = self._data_pack
