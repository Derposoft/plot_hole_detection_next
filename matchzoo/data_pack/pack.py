...
def pack(df: pd.DataFrame) -> "matchzoo.DataPack":
    ...
    if "text_left" not in df or "text_right" not in df:
        raise ValueError("Input data frame must have `text_left` and `text_right`.")
    ...
    right = _merge(
        df, id_right, "text_right", "id_right", "raw_text_right", "images_right"
    )
...
def _merge(
    data: pd.DataFrame,
    ids: typing.Union[list, np.array],
    text_label: str,
    id_label: str,
    raw_text_label: str,
    images_path_label: str,
):
    left = pd.DataFrame(
        data={
            text_label: data[text_label],
            id_label: ids,
            raw_text_label: data[raw_text_label],
            images_path_label: data[images_path_label],  # added by Ben
        }
    )
