...
def load_glove_embedding(
    term_index: mz.preprocessors.units.Vocabulary.TermIndex = None, dimension: int = 50
    ...
    return mz.embedding.load_from_file(
        file_path=str(file_path), mode="glove", term_index=term_index
    )


def load_glove_embedding_matching(
    ...
    **kargs
    ...
    return mz.embedding.load_from_file_matching(
        file_path=str(file_path), mode="glove", term_index=term_index, **kargs
    )


def load_glove_embedding_FC(
    term_index: mz.preprocessors.units.Vocabulary.TermIndex = None,
    dimension: int = 50,
    **kargs
) -> mz.embedding.Embedding:
    ...
    return mz.embedding.load_from_file_FC(
        file_path=str(file_path), mode="glove", term_index=term_index, **kargs
    )
