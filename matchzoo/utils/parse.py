...
def parse_metric(
    metric: typing.Union[str, typing.Type[BaseMetric], BaseMetric], task: str
) -> BaseMetric:
    """
    ...
    """
    if task is None:
        raise ValueError("Should specify one `BaseTask`.")
    if task == "ranking":
        return _parse_metric(metric, RankingMetric)
    if task == "classification":
        return _parse_metric(metric, ClassificationMetric)
    else:
        raise ValueError("Should be a Ranking or Classification task.")
...
def parse_optimizer(
    ...
    if isinstance(identifier, str):
        ...
        raise ValueError("Could not interpret optimizer identifier: " + str(identifier))
