...
class MatchingHistogram(Unit):
    ...
    def _normalize_embedding(self):
        ...
        l2_norm = np.sqrt((self._embedding_matrix * self._embedding_matrix).sum(axis=1))
        self._embedding_matrix = self._embedding_matrix / l2_norm[:, np.newaxis]

    def transform(self, input_: list) -> list:
        """Transform the input text."""
        text_left, text_right = input_
        matching_hist = np.ones((len(text_left), self._hist_bin_size), dtype=np.float32)
        ...
        for (i, j), value in np.ndenumerate(matching_matrix):
            bin_index = int((value + 1.0) / 2.0 * (self._hist_bin_size - 1.0))
