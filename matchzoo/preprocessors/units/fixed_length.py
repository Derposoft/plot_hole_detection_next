...
class FixedLength(Unit):
    ...
    def transform(self, input_: list) -> list:
        ...
        fixed_tokens = np.full(
            [self._text_length], self._pad_value, dtype=np_tokens.dtype
        )
        ...
        if self._truncate_mode == "pre":
            ...
            raise ValueError(
                "{} is not a vaild " "truncate mode.".format(self._truncate_mode)
            )
        ...
        if self._pad_mode == "post":
            ...
            raise ValueError("{} is not a vaild " "pad mode.".format(self._pad_mode))
