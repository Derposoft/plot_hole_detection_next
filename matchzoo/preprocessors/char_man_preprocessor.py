...
class CharManPreprocessor(BasePreprocessor):
    ...
    def __init__(
        ...
        self._left_char_src_fixedlength_unit = units.FixedLength(
            self._fixed_length_left_src, pad_mode="post"
        )
        self._right_char_src_fixedlength_unit = units.FixedLength(
            self._fixed_length_right_src, pad_mode="post"
        )
    ...
    def fit(self, data_pack: DataPack, verbose: int = 1):
        ...
        claim_source_unit = build_entity_unit(
            column="claim_source", data_pack=data_pack, mode="left"
        )
        article_source_unit = build_entity_unit(
            column="evidence_source", data_pack=data_pack, mode="right"
        )
        self._context["claim_source_unit"] = claim_source_unit
        self._context["article_source_unit"] = article_source_unit

        char_source_unit = build_ngram_unit(
            left_column="claim_source",
            right_column="evidence_source",
            data_pack=data_pack,
            mode="both",
        )
    ...
    def transform(self, data_pack: DataPack, verbose: int = 1) -> DataPack:
        ...
        def map_claim_source(entity: str):
            return self._context["claim_source_unit"].transform([entity])

        def map_evidence_source(entity: str):
            return self._context["article_source_unit"].transform([entity])

        def map_src2char(entity: str):
            return self._context["char_source_unit"].transform(list(entity))

        data_pack.left["claim_source"] = data_pack.left["claim_source"].progress_apply(
            map_claim_source
        )
        data_pack.left["char_claim_source"] = data_pack.left[
            "char_claim_source"
        ].progress_apply(map_src2char)
        data_pack.left["char_claim_source"] = data_pack.left[
            "char_claim_source"
        ].progress_apply(self._left_char_src_fixedlength_unit.transform)

        data_pack.right["evidence_source"] = data_pack.right[
            "evidence_source"
        ].progress_apply(map_evidence_source)
        data_pack.right["char_evidence_source"] = data_pack.right[
            "char_evidence_source"
        ].progress_apply(map_src2char)
        data_pack.right["char_evidence_source"] = data_pack.right[
            "char_evidence_source"
        ].progress_apply(self._right_char_src_fixedlength_unit.transform)

        data_pack.apply_on_text(
            chain_transform(self._units), inplace=True, verbose=verbose
        )
...
def build_entity_unit(
    column: str, data_pack: DataPack, mode: str = "both", verbose: int = 1
) -> Vocabulary:
    """
    ...
    if verbose:
        description = (
            "Building Entities " + unit.__class__.__name__ + " from a datapack."
        )
...
def build_ngram_unit(
    ...
    if verbose:
        description = (
            "Building Characters " + unit.__class__.__name__ + " from a datapack."
        )
        corpus = tqdm(corpus, desc=description)
    unit.fit(corpus)
    return unit
