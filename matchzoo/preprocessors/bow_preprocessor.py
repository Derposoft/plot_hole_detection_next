...
class BoWPreprocessor(BasePreprocessor):
    ...
    def __init__(
        ...
        self._images_unit = ImagesUnit(
            right_visual_features_pth, fixed_num_images_right
        )
    ...
    def fit(self, data_pack: DataPack, verbose: int = 1):
        ...
        fitted_filter_unit = build_unit_from_data_pack(
            self._filter_unit, data_pack, flatten=False, mode="right", verbose=verbose
        )
        data_pack = data_pack.apply_on_text(
            fitted_filter_unit.transform, mode="right", verbose=verbose
        )
        self._context["filter_unit"] = fitted_filter_unit

        vocab_unit = build_vocab_unit(
            data_pack, verbose=verbose, mode="right"
        )  # only rely on the right side
    ...
    def transform(self, data_pack: DataPack, verbose: int = 1) -> DataPack:
        ...
        data_pack.apply_on_text(
            self._context["filter_unit"].transform,
            mode="right",
            inplace=True,
            verbose=verbose,
        )
        ...
        data_pack.apply_on_text(
            convert_to_bow, mode="both", inplace=True, verbose=verbose
        )
        data_pack.right["images_right"] = data_pack.right[
            "images_right"
        ].progress_apply(self._images_unit.transform)
        return data_pack
...
class ImagesUnit(Unit):
    def __init__(self, visual_features_pth: str, max_len_images: int):
        ...
        self.mapper = dict(
            zip(paths, range(len(paths)))
        )  # indices here are rows of `fat_tensor`
    ...
    def transform(self, images: str) -> list:
        ...
        """
        images = [
            self.fat_tensor[self.mapper[p]].numpy().tolist()
            for p in images[: self.max_len_images]
        ]
