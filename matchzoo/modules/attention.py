...
class BidirectionalAttention(nn.Module):
    ...
    def forward(self, v1, v1_mask, v2, v2_mask):
        ...
        v2_v1_attn = F.softmax(
            similarity_matrix.masked_fill(v1_mask.unsqueeze(2), -1e-7), dim=1
        )
        v1_v2_attn = F.softmax(
            similarity_matrix.masked_fill(v2_mask.unsqueeze(1), -1e-7), dim=2
        )
...
class MatchModule(nn.Module):
    ...
    def forward(self, v1, v2, v2_mask):
        ...
        v1_v2_attn = F.softmax(
            similarity_matrix.masked_fill(v2_mask.unsqueeze(1).bool(), -1e-7), dim=2
        )
