...
def compute_average_classification_results(
    kfold_results: List[Dict[str, float]], metrics: List[str], **kargs
    ...
    targets = [
        KeyWordSettings.AUC_metric,
        KeyWordSettings.F1_macro,
        KeyWordSettings.F1_micro,
        KeyWordSettings.F1TrueCls,
        KeyWordSettings.PrecisionTrueCls,
        KeyWordSettings.RecallTrueCls,
        KeyWordSettings.F1FalseCls,
        KeyWordSettings.PrecisionFalseCls,
        KeyWordSettings.RecallFalseCls,
        KeyWordSettings.F1MixedCls,
        KeyWordSettings.PrecisionMixedCls,
        KeyWordSettings.RecallMixedCls,
    ]
    ls = ["ModelName"] + [
        template % (avg_results[key]["avg"], avg_results[key]["std"]) for key in targets
    ]
    latex = "&".join(ls)
