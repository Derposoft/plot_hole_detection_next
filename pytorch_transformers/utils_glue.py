...
class InputExample(object):
    ...
    def __init__(
        ...
        text_b=None,
        label=None,
        tokenized_text_a: List[str] = None,
        tokenized_text_b: List[str] = None,
    ):
...
class SearchProcessor(DataProcessor):
    ...
    def get_dev_examples(self, data_dir, tokenizer: PreTrainedTokenizer = None):
        """See base class."""
        return self._create_examples(
            self._read_tsv2(os.path.join(data_dir, "Snopes.dev.tsv")),
            "dev",
            tokenizer=tokenizer,
        )
    ...
    def _read_tsv2(
        cls, input_file, quotechar=None, tokenizer: PreTrainedTokenizer = None
    ):
        """Reads a tab separated value file."""
        with open(input_file, "r", encoding="utf-8-sig") as f:
            reader = csv.reader(f, delimiter="\t", quotechar=quotechar)
