...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, attn_mask=None, head_mask=None):
        ...
        sa_output = self.attention(
            query=x, key=x, value=x, mask=attn_mask, head_mask=head_mask
        )
        if self.output_attentions:
            (
                sa_output,
...
class DistilBertModel(DistilBertPreTrainedModel):
    ...
    def forward(self, input_ids, attention_mask=None, head_mask=None):
        ...
        tfmr_output = self.transformer(
            x=embedding_output, attn_mask=attention_mask, head_mask=head_mask
        )
...
class DistilBertForMaskedLM(DistilBertPreTrainedModel):
    ...
    def tie_weights(self):
        ...
        self._tie_or_clone_weights(
            self.vocab_projector, self.distilbert.embeddings.word_embeddings
        )

    def forward(
        self, input_ids, attention_mask=None, masked_lm_labels=None, head_mask=None
    ):
