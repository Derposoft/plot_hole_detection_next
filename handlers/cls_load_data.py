...
def load_data(
    ...
    """
    if stage not in ["dev"] + ["train_%s" % i for i in range(kfolds)] + [
        "test_%s" % i for i in range(kfolds)
    ]:
        raise ValueError(
            "%s is not a valid stage. Must be one of `train`, `dev`, and `test`."
            % stage
        )
    ...
    df = pd.DataFrame(
        {
            ...
            "extended_text": table.progress_apply(
                lambda x: merge_text(x.claim_id, x.claim_text), axis=1
            ),
