...
class Sampler(object):
    ...
    def get_train_instances(self, interactions, num_negatives: int):
        ...
        negative_samples = np.zeros(
            (query_ids.shape[0], num_negatives, interactions.padded_doc_length),
            np.int64,
        )
        ...
        return (
            ...
            negative_docs_ids,
            negative_samples,
            negative_samples_lens,
        )

    def get_train_instances_declare(
        self,
        ...
        """
        claim_sources = np.array(
            [interactions.dict_claim_source[e] for e in interactions.claims]
        )
        evidence_sources = np.array(
            [interactions.dict_evd_source[e] for e in interactions.evidences]
        )
        return (
            interactions.claims,
            interactions.claims_contents,
            interactions.claims_lens,
            claim_sources,
            interactions.evidences,
            interactions.evd_contents,
            interactions.evd_lens,
            evidence_sources,
            interactions.pair_labels,
        )

    def get_train_instances_hanfc(
        self,
        interactions: interactions.ClassificationInteractions,
        fixed_num_evidences: int,
    ):
        ...
        evd_docs_ids = (
            np.zeros((query_ids.shape[0], fixed_num_evidences), np.int64) - 1
        )  # all indices are -1
        # by default it is all pad tokens
        evd_docs_contents = np.zeros(
            (query_ids.shape[0], fixed_num_evidences, interactions.padded_doc_length),
            np.int64,
        )
        evd_docs_lens = np.zeros((query_ids.shape[0], fixed_num_evidences), np.int64)
        evd_sources = (
            np.zeros((query_ids.shape[0], fixed_num_evidences), np.int64) - 1
        )  # for padding sources are -1
        evd_cnt_each_query = np.zeros((query_ids.shape[0]), np.int64)

        for i, u in enumerate(query_ids):
            evidences_info = interactions.dict_claims_and_evidences_test[
                u
            ]  # use u not i
            assert len(evidences_info) <= fixed_num_evidences
            evd_cnt_each_query[i] = len(
                evidences_info[0]
            )  # number of real evidences for the query i
            # we have a list of evidences, now I need to take the content and doc_id
            for idx, (doc_id, doc_label, doc_content, doc_len) in enumerate(
                zip(*evidences_info)
            ):
                evd_docs_contents[i][
                    idx
                ] = doc_content  # we already pad the content array with zeros due to init
                evd_docs_lens[i][idx] = doc_len  # we set 0 length for padding evidences
                evd_docs_ids[i][
                    idx
                ] = doc_id  # we set -1 as index for padding evidences
                evd_sources[i][idx] = interactions.dict_evd_source[doc_id][
                    0
                ]  # -1 since we have an array size 1
    ...
    def get_train_instances_char_man(
        self,
        interactions: interactions.ClassificationInteractions,
        fixed_num_evidences: int,
    ):
        ...
        evd_cnt_each_query = np.zeros((query_ids.shape[0]), np.int64)
        evd_docs_char_source_contents = np.zeros(
            (
                query_ids.shape[0],
                fixed_num_evidences,
                interactions.padded_doc_char_source_length,
            ),
            np.int64,
        )
        evd_docs_adj = np.zeros(
            (
                query_ids.shape[0],
                fixed_num_evidences,
                interactions.padded_doc_length,
                interactions.padded_doc_length,
            ),
            np.float,
        )
        ...
        for i, u in enumerate(query_ids):
            ...
            # we have a list of evidences, now I need to take the content and doc_id
            for idx, (doc_id, doc_label, doc_content, doc_len, docs_adj) in enumerate(
                zip(*evidences_info)
            ):
                evd_docs_contents[i][
                    idx
                ...
                evd_docs_char_source_contents[i][
                    idx
                ] = interactions.dict_char_right_src[doc_id]
