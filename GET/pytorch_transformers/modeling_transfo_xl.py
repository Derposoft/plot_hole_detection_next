...
class RelLearnableMultiHeadAttn(RelMultiHeadAttn):
    ...
    def forward(
        self, w, r_emb, r_w_bias, r_bias, attn_mask=None, mems=None, head_mask=None
    ):
    ...
    def __init__(self, config):
        ...
        if self.attn_type == 0:  # default attention
            ...
            self.r_emb = nn.Parameter(
                torch.FloatTensor(self.n_layer, self.max_klen, self.n_head, self.d_head)
            )
            self.r_bias = nn.Parameter(
                torch.FloatTensor(self.n_layer, self.max_klen, self.n_head)
            )
