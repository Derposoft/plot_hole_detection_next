...
from fitting.FittingFC.multi_level_attention_composite_fitter import (
    MultiLevelAttentionCompositeFitter,
)
...
class CharManFitterQueryRepr1(MultiLevelAttentionCompositeFitter):
    ...
    def __init__(
        ...
        **kargs
        ...
        super(CharManFitterQueryRepr1, self).__init__(
            net,
            loss,
            n_iter,
            testing_epochs,
    ...
    def fit(
        ...
        for epoch_num in range(self._n_iter):
            ...
            (
                ...
                pair_labels,
                evd_docs_adj,
            ) = self._sampler.get_train_instances_char_man(
                train_iteractions, self.fixed_num_evidences
            )
            ...
            for minibatch_num, (
                ...
                batch_query = my_utils.gpu(
                    torch.from_numpy(batch_query), self._use_cuda
                )
                ...
                batch_query_sources = my_utils.gpu(
                    torch.from_numpy(batch_query_sources), self._use_cuda
                )
                batch_query_chr_src = my_utils.gpu(
                    torch.from_numpy(batch_query_chr_src), self._use_cuda
                )
                batch_query_adj = my_utils.gpu(
                    torch.from_numpy(batch_query_adj), self._use_cuda
                )

                batch_evd_docs = my_utils.gpu(
                    torch.from_numpy(batch_evd_docs), self._use_cuda
                )
                batch_evd_contents = my_utils.gpu(
                    torch.from_numpy(batch_evd_contents), self._use_cuda
                )
                # batch_evd_lens = my_utils.gpu(torch.from_numpy(batch_evd_lens), self._use_cuda)
                batch_evd_sources = my_utils.gpu(
                    torch.from_numpy(batch_evd_sources), self._use_cuda
                )
                batch_evd_cnt_each_query = my_utils.gpu(
                    torch.from_numpy(batch_evd_cnt_each_query), self._use_cuda
                )
                batch_evd_chr_src = my_utils.gpu(
                    torch.from_numpy(batch_evd_chr_src), self._use_cuda
                )

                batch_labels = my_utils.gpu(
                    torch.from_numpy(batch_labels), self._use_cuda
                )
                batch_evd_docs_adj = my_utils.gpu(
                    torch.from_numpy(batch_evd_docs_adj), self._use_cuda
                )
                # total_pairs += self._batch_size * self.
                additional_data = {
                    KeyWordSettings.EvidenceCountPerQuery: batch_evd_cnt_each_query,
            ...
            if verbose:  # validation after each epoch
                ...
                if (
                    ...
                    and count_patience_epochs > self._early_stopping_patience
                ):
    ...
    def _flush_training_results(self, best_val_f1_macro: float, best_epoch: int):
        ...
        self.output_handler.myprint(
            "Best result: | vad F1_macro = %.5f | epoch = %d"
            % (best_val_f1_macro, best_epoch)
        )

    def _get_multiple_evidences_predictions_normal(
        self,
        ...
        **kargs
        ...
        for (
            ...
            expaned_labels.extend(
                [int(torch_utils.cpu(label).detach().numpy())] * evd_cnt
            )
        # concat
        ...
        predictions = self._net(
            query_contents, evd_doc_contents, **additional_paramters
        )
    ...
    def evaluate(
        self,
        testRatings: interactions.ClassificationInteractions,
        K: int,
        ...
        **kargs
        ...
        for query, evidences_info in testRatings.dict_claims_and_evidences_test.items():
            ...
            evd_sources = np.array(
                [testRatings.dict_evd_source[e] for e in evd_ids]
            )  # (len(labels), 1)
            evd_sources = self._pad_article_sources(evd_sources)  # (1, 30)
            evd_char_src = np.array(
                [testRatings.dict_char_right_src[e] for e in evd_ids]
            )  # (len(labels), 1)
            query_len = np.array(
                [testRatings.dict_claim_lengths[query]]
            )  # shape = (1, ) where B =1
            # doc_lens = [testRatings.dict_doc_lengths[d] for d in docs]

            claim_content = np.tile(claim_content, (1, 1))  # (1, L)
            L = claim_content.shape[1]
            evd_contents = np.array(evd_contents).reshape(
                1, len(labels), -1
            )  # shape = (1, n, R)
            ...
            claim_content = my_utils.gpu(
                my_utils.numpy2tensor(claim_content, dtype=torch.int), self._use_cuda
            )
            evd_contents = my_utils.gpu(
                my_utils.numpy2tensor(evd_contents, dtype=torch.int), self._use_cuda
            )  # (1, x, R)
            padded_evd_contents = my_utils.gpu(
                my_utils.numpy2tensor(padded_evd_contents, dtype=torch.int),
                self._use_cuda,
            )  # (1, x, R)
            ...
            (
                d_new_indices,
                d_old_indices,
            ) = torch_utils.get_sorted_index_and_reverse_index(evd_lengths)
            evd_lengths = my_utils.gpu(
                my_utils.numpy2tensor(evd_lengths, dtype=torch.int), self._use_cuda
            )
            ...
            # for sources
            claim_source = my_utils.gpu(
                my_utils.numpy2tensor(claim_source, dtype=torch.int), self._use_cuda
            )
            evd_sources = my_utils.gpu(
                my_utils.numpy2tensor(evd_sources, dtype=torch.int), self._use_cuda
            )
            claim_char_src = my_utils.gpu(
                my_utils.numpy2tensor(claim_char_src, dtype=torch.int), self._use_cuda
            )
            evd_char_src = my_utils.gpu(
                my_utils.numpy2tensor(evd_char_src, dtype=torch.int), self._use_cuda
            )
            ...
            evd_adj = np.array(
                [testRatings.dict_doc_adj[e] for e in evd_ids]
            )  # (n, R, R)
            ...
            additional_information = {
                ...
                KeyWordSettings.DocContentNoPaddingEvidence: evd_contents.view(
                    1 * len(labels), -1
                ),  # (B1, R)
                KeyWordSettings.FIXED_NUM_EVIDENCES: self.fixed_num_evidences,
                KeyWordSettings.EvidenceCountPerQuery: torch_utils.gpu(
                    torch.from_numpy(np.array([len(labels)])), self._use_cuda
                ),
                KeyWordSettings.QueryContentNoPaddingEvidence: claim_content.expand(
                    len(labels), L
                ),
            ...
            probs = self._net.predict(
                claim_content, padded_evd_contents, **additional_information
            )  # shape = (len(labels), )
            if output_ranking:
                ...
                list_error_analysis.append(
                    self._prepare_error_analysis(
                        testRatings, query, evd_ids, probs, labels, **more_info
                    )
                )
            else:
                predictions = probs.argmax(dim=1)
            all_final_preds.append(
                float(my_utils.cpu(predictions).detach().numpy().flatten())
            )
            all_final_probs.append(
                float(my_utils.cpu(probs[:, 1]).detach().numpy().flatten())
            )

        results = self._computing_metrics(
            true_labels=all_labels,
            predicted_labels=all_final_preds,
            predicted_probs=all_final_probs,
        )
        if output_ranking:
            return (
                results,
                list_error_analysis,
            )  # sorted(list_error_analysis, key=lambda x: x["qid"])
        return results

    def _computing_metrics(
        self,
        ...
        fpr, tpr, thresholds = sklearn.metrics.roc_curve(
            true_labels, predicted_probs, pos_label=1
        )
        ...
        precision_true_class = precision_score(
            true_labels, predicted_labels, labels=[1], average=None
        )[0]
        recall_true_class = recall_score(
            true_labels, predicted_labels, labels=[1], average=None
        )[0]
        f1_true_class = f1_score(
            true_labels, predicted_labels, labels=[1], average=None
        )[0]

        precision_false_class = precision_score(
            true_labels, predicted_labels, labels=[0], average=None
        )[0]
        recall_false_class = recall_score(
            true_labels, predicted_labels, labels=[0], average=None
        )[0]
        f1_false_class = f1_score(
            true_labels, predicted_labels, labels=[0], average=None
        )[0]

        precision_mixed_class = (
            precision_score(true_labels, predicted_labels, labels=[2], average=None)[0]
            if self.output_size == 3
            else 0.0
        )
        recall_mixed_class = (
            recall_score(true_labels, predicted_labels, labels=[2], average=None)[0]
            if self.output_size == 3
            else 0.0
        )
        f1_mixed_class = (
            f1_score(true_labels, predicted_labels, labels=[2], average=None)[0]
            if self.output_size == 3
            else 0.0
        )
    ...
    def _prepare_error_analysis(
        ...
        **kargs
        ...
        word_att_weights, evd_att_weight = kargs[
            KeyWordSettings.FCClass.AttentionWeightsInfo
        ]  # () vs. (1, 30, num_heads)
    ...
    def _output_results_every_epoch(
        self,
        topN: int,
        ...
        assert (
            ...
            in KeyWordSettings.ClaimCountVal
        ), len(val_interactions.dict_claims_and_evidences_test)
        ...
        if epoch_num and epoch_num % self._testing_epochs == 0:
            ...
            assert (
                ...
                in KeyWordSettings.ClaimCountTest
            ), len(test_interactions.dict_claims_and_evidences_test)
            ...
            TensorboardWrapper.mywriter().add_scalar(
                "f1/f1_macro_test", f1_macro_test, epoch_num
            )
            TensorboardWrapper.mywriter().add_scalar(
                "f1/f1_micro_test", f1_micro_test, epoch_num
            )
            ...
            self.output_handler.myprint(
                "|Epoch %03d | Test F1_macro = %.5f | Testing time: %04.1f(s)"
                % (epoch_num, f1_macro_test, testing_time)
            )

        # TensorboardWrapper.mywriter().add_scalar("auc/auc_val", auc_val, epoch_num)
        TensorboardWrapper.mywriter().add_scalar(
            "f1/f1_macro_val", f1_macro_val, epoch_num
        )
        TensorboardWrapper.mywriter().add_scalar(
            "f1/f1_micro_val", f1_micro_val, epoch_num
        )
    ...
    def load_best_model(
        ...
        result_val, error_analysis_val = self.evaluate(
            val_interactions, topN, output_ranking=True
        )
        auc_val = result_val[KeyWordSettings.AUC_metric]
        f1_val = result_val[KeyWordSettings.F1]
        f1_macro_val = result_val[KeyWordSettings.F1_macro]
        ...
        result_test, error_analysis_test = self.evaluate(
            test_interactions, topN, output_ranking=True
        )
        auc_test = result_test[KeyWordSettings.AUC_metric]
        f1_test = result_test[KeyWordSettings.F1]
        ...
        self.output_handler.save_error_analysis_validation(
            json.dumps(error_analysis_val, sort_keys=True, indent=2)
        )
        self.output_handler.save_error_analysis_testing(
            json.dumps(error_analysis_test, sort_keys=True, indent=2)
        )
        self.output_handler.myprint(
            "Best Vad F1_macro = %.5f | Best Vad AUC = %.5f"
