...
class DeclareFitter(DenseBaselineFitter):
    def __init__(
        ...
        **kargs
        ...
        super(DeclareFitter, self).__init__(
            net,
            loss,
            n_iter,
            testing_epochs,
    ...
    def fit(
        ...
        for epoch_num in range(self._n_iter):
            ...
            (
                ...
                evd_sources,
                pair_labels,
            ) = self._sampler.get_train_instances_declare(
            ...
            (
                ...
                query_content,
                query_lengths,
                query_sources_shuffled,
                evd_docs,
                evd_docs_contents,
                evd_docs_lens,
                evd_sources_shuffled,
                pair_labels_shuffled,
            ) = my_utils.shuffle(
                query_ids,
                left_contents,
                left_lengths,
            ...
            for minibatch_num, (
                ...
                batch_query_content,
                batch_query_len,
                batch_query_sources,  # i.e. claim source
                batch_evd_docs,
                batch_evd_contents,
                batch_evd_lens,
                batch_evd_sources,
            ...
            if verbose:  # validation after each epoch
                f1_macro_val, auc_val = self._output_results_every_epoch(
                    topN,
                    val_interactions,
                    test_interactions,
                    epoch_num,
                ...
                if (f1_macro_val > best_val_f1_macro) or (
                    f1_macro_val == best_val_f1_macro and auc_val > best_val_auc
                ):  # prioritize f1_macro
                    ...
                    best_val_auc, best_val_f1_macro, best_epoch = (
                        auc_val,
                        f1_macro_val,
                        epoch_num,
                    )
    ...
    def _flush_training_results(self, best_val_auc: float, best_epoch: int):
        ...
        self.output_handler.myprint(
            "Best result: | vad auc = %.5f | epoch = %d" % (best_val_auc, best_epoch)
        )
    ...
    def _output_results_every_epoch(
        ...
        if epoch_num and epoch_num % self._testing_epochs == 0:
            ...
            testing_time = t2 - t1
            TensorboardWrapper.mywriter().add_scalar(
                "auc/auc_test", auc_test, epoch_num
            )
            TensorboardWrapper.mywriter().add_scalar(
            ...
            self.output_handler.myprint(
                "|Epoch %03d | Test AUC = %.5f | Testing time: %04.1f(s)"
    ...
    def _get_multiple_evidences_predictions_normal(
        ...
        **kargs
        ...
        q_new_indices, q_old_indices = torch_utils.get_sorted_index_and_reverse_index(
            query_lens
        )
    ...
    def load_best_model(
        ...
        self.output_handler.myprint(
            "Best Vad AUC = %.5f | Best Vad F1 = %.5f "
    ...
    def evaluate(
        ...
        **kargs
        ...
        for query, evidences_info in testRatings.dict_claims_and_evidences_test.items():
            ...
            claim_source = np.array(
                [testRatings.dict_claim_source[query]] * len(labels)
            )  # (len(labels), 1)
            ...
            claim_content = np.tile(
                claim_content, (len(labels), 1)
            )  # len(labels), query_contnt_leng)
            evd_contents = np.array(
                evd_contents
            )  # shape = (real_num_evd, R) or = (len(labels), R)
            ...
            (
                q_new_indices,
                q_old_indices,
            ) = torch_utils.get_sorted_index_and_reverse_index(query_len)
            evd_lengths = my_utils.gpu(
                my_utils.numpy2tensor(evd_lengths, dtype=torch.int), self._use_cuda
            )
            query_len = my_utils.gpu(
                my_utils.numpy2tensor(query_len, dtype=torch.int), self._use_cuda
            )
            ...
            query_id_tsr = torch_utils.gpu(
                torch.from_numpy(np.array([query])), self._use_cuda
            )
            additional_information = {
                ...
                KeyWordSettings.DocIDs: torch_utils.gpu(
                    torch.from_numpy(np.array(evd_ids)), self._use_cuda
                ),
            }
            probs = self._net.predict(
                claim_content, evd_contents, **additional_information
            )  # shape = (len(labels), )
            all_final_probs.append(
                float(my_utils.cpu(probs).detach().numpy().flatten())
            )

        results = self._computing_metrics(
            true_labels=all_labels, predicted_probs=all_final_probs
        )
        if output_ranking:
    ...
    def _computing_metrics(self, true_labels: List[int], predicted_probs: List[float]):
        ...
        precision_true_class = precision_score(
            true_labels, predicted_labels, pos_label=1
        )
        ...
        precision_false_class = precision_score(
            true_labels, predicted_labels, pos_label=0
        )
