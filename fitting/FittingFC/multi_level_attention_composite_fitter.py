...
class MultiLevelAttentionCompositeFitter(DeclareFitter):
    ...
    def fit(
        ...
        for epoch_num in range(self._n_iter):
            ...
            (
                ...
                evd_cnt_each_query,
                pair_labels,
            ) = self._sampler.get_train_instances_hanfc(
    ...
    def _get_multiple_evidences_predictions_normal(
        ...
        **kargs
        ...
        expaned_labels = []
        for evd_cnt, q_cont, q_len, evd_lens, evd_doc_cont, label in zip(
            evd_count_per_query,
            query_contents,
            query_lens,
            evd_docs_lens,
        ...
        (
            q_new_indices,
            q_restoring_indices,
        ) = torch_utils.get_sorted_index_and_reverse_index(query_lens)
    ...
    def _prepare_error_analysis(
        ...
        **kargs
        ...
        q_details = {
            ...
            KeyWordSettings.FCClass.PredictedProb: float(
                my_utils.cpu(probs).detach().numpy().flatten()
            ),
    ...
    def _pad_evidences(self, original_evd_contents: np.ndarray):
        ...
        padded_evd_contents = np.concatenate(
            [original_evd_contents, padding_zeros], axis=1
        )
    ...
    def _pad_article_sources(self, evd_sources: np.ndarray):
        ...
        cnt_pads = self.fixed_num_evidences - num_evidences
        padding_minus_1 = (
            np.zeros((batch_size, cnt_pads)).astype(np.int64) - 1
        )  # padding sources
