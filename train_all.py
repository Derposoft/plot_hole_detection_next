...
def fit_models(args):
    ...
    for i in range(args.num_folds):
        ...
        logfolder_result_per_fold = os.path.join(
            outfolder_per_fold, "result_%s.txt" % int(seed)
        )
        ...
        predict_pack = cls_load_data.load_data(
            root + "/%sfold" % args.num_folds, "test_%s" % i, kfolds=args.num_folds
        )
        train_pack = cls_load_data.load_data(
            root + "/%sfold" % args.num_folds, "train_%s" % i, kfolds=args.num_folds
        )
        ...
        file_handler.myprint(
            "Min query length, "
            + str(min_query_length)
            + " Min doc length "
            + str(min_doc_length)
        )
        file_handler.myprint(
        ...
        train_interactions = ClassificationInteractions(
            train_processed, **additional_data
        )
        valid_interactions = ClassificationInteractions(
            valid_processed, **additional_data
        )
        test_interactions = ClassificationInteractions(
            predict_processed, **additional_data
        )
        ...
        glove_embedding = mz.datasets.embeddings.load_glove_embedding_FC(
            dimension=args.word_embedding_size, term_index=term_index, **additional_data
        )

        embedding_matrix = glove_embedding.build_matrix(term_index)
        entity_embs1 = entity_embedding.EntityEmbedding(args.claim_src_emb_size)
        claim_src_embs_matrix = entity_embs1.build_matrix(
            preprocessor.context["claim_source_unit"].state["term_index"]
        )

        entity_embs2 = entity_embedding.EntityEmbedding(args.article_src_emb_size)
        article_src_embs_matrix = entity_embs2.build_matrix(
            preprocessor.context["article_source_unit"].state["term_index"]
        )
        ...
        match_params[
            "num_att_heads_for_words"
        ] = args.num_att_heads_for_words  # first level
        match_params[
            "num_att_heads_for_evds"
        ] = args.num_att_heads_for_evds  # second level
        ...
        match_model = graph_based_semantic_structure.Graph_basedSemantiStructure(
            match_params
        )

        file_handler.myprint("fitting Model")
        fit_model = char_man_fitter_query_repr1.CharManFitterQueryRepr1(
            net=match_model,
            loss=args.loss_type,
            n_iter=args.epochs,
        ...
        try:
            ...
            dev_results, test_results = fit_model.load_best_model(
                valid_interactions, test_interactions
            )
    ...
    avg_test_results = evaluator.compute_average_classification_results(
        kfold_test_results, list_metrics, **additional_data
    )
    ...
    result_json_path = os.path.join(
        secondary_log_folder, "avg_5fold_result_%s.json" % int(seed)
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--path",
        default="../test_code/ben_data_10_no_body",
        help="Input data path",
        type=str,
    )
    parser.add_argument(
        "--dataset", type=str, default="Snopes", help="[Snopes, Politifact]"
    )
    parser.add_argument(
        "--epochs", default=100, help="Number of epochs to run", type=int
    )
    ...
    parser.add_argument(
        ...
        default=10,
        type=int,
        help="The number of step to stop training",
    )
    parser.add_argument("--debug", default=1, type=int, help="debug or not")
    parser.add_argument(
        "--log", default="", type=str, help="folder for logs and saved models"
    )
    parser.add_argument(
        "--optimizer", nargs="?", default="adam", help="Specify an optimizer: adam"
    )
    ...
    parser.add_argument(
        "--word_embedding_size",
        default=300,
        help="the dimensions of word embeddings",
        type=int,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="how many steps to decay the learning rate",
    )
    parser.add_argument(
        "--decay_weight", type=float, default=0.0001, help="percent of decaying"
    )
    parser.add_argument(
        "--fixed_length_left", type=int, default=30, help="Maximum length of each query"
    )
    parser.add_argument(
        "--fixed_length_right",
        type=int,
        default=100,
        help="Maximum length of each document",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=30,
        help="Max number of evidences each claim",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Using claim source embedding or not",
    )
    parser.add_argument(
        "--use_article_source",
        type=int,
        default=1,
        help="Using article source embedding or not",
    )
    parser.add_argument(
        "--num_att_heads_for_words",
        type=int,
        default=1,
        help="Nums att heads for words",
    )
    parser.add_argument(
        "--num_att_heads_for_evds",
        type=int,
        default=1,
        help="Nums att heads for evidences",
    )

    # GNN related
    parser.add_argument(
        "--gnn_window_size", type=int, default=2, help="GNN window size"
    )
    parser.add_argument(
        "--gnn_dropout", type=float, default=0.5, help="Dropout rate for GNN"
    )
    parser.add_argument(
        "--gsl_rate", type=float, default=0.8, help="Dropout rate of GSL"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.2,
        help="Dropout rate for word embs in claim",
    )
    parser.add_argument(
        "--dropout_right",
        type=float,
        default=0.2,
        help="Dropout rate for word embs in articles",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--article_src_emb_size",
        type=int,
        default=128,
        help="Embedding size of article",
    )
    parser.add_argument(
        "--claim_src_emb_size", type=int, default=128, help="Embedding size of article"
    )
    parser.add_argument(
        "--fixed_length_left_src_chars",
        type=int,
        default=20,
        help="Max number of chars for left sources",
    )
    parser.add_argument(
        "--fixed_length_right_src_chars",
        type=int,
        default=20,
        help="Max number of chars for right sources",
    )
