...
class MatchInteraction(object):
    ...
    def __init__(self, data_pack: matchzoo.DataPack, **kargs):
        ...
        assert len(self.unique_doc_ids) == len(
            set(self.unique_doc_ids)
        ), "Must be unique ids for doc ids"
        assert len(self.unique_query_ids) != len(
            self.unique_doc_ids
        ), "Impossible to have equal number of docs and number of original tweets"

        (
            self.pos_queries,
            self.pos_docs,
            self.negatives,
            self.unique_queries_test,
    ...
    def convert_leftright(
        ...
        **kargs
        ...
        for (
            ...
            part.iterrows()
        ):  # very dangerous, be careful because it may change order!!!
            ids.append(index)
    ...
    def convert_relations(self, relation: pd.DataFrame):
        ...
        for index, row in relation.iterrows():
            ...
            unique_queries[query] = unique_queries.get(
                query, [[], [], [], []]
            )  # doc, label, content, length
...
class BaseClassificationInteractions(object):
    ...
    def __init__(self, data_pack: matchzoo.DataPack, **kargs):
        ...
        additional_field = {
            KeywordSettings.FCClass.CharSourceKey: "char_claim_source",
        ...
        (
            ...
            raw_source_key="raw_claim_source",
            **additional_field
        )
        self.data_pack = data_pack
        assert len(self.unique_query_ids) == len(
        ...
        """ Why do I need to sort it? I have no idea why did I do it? """
        additional_field = {
            KeywordSettings.FCClass.CharSourceKey: "char_evidence_source",
            KeywordSettings.GNN_Window: kargs[KeywordSettings.GNN_Window],
        }
        (
            self.unique_doc_ids,
...
class ClassificationInteractions(BaseClassificationInteractions):
    ...
    def __init__(self, data_pack: matchzoo.DataPack, **kargs):
        ...
        (
            self.claims,
            self.claims_labels,
            self.dict_claims_and_evidences_test,
        ) = self.convert_relations(data_pack.relation)

        # for queries, padded
        self.np_query_contents = np.array(
            [self.dict_claim_contents[q] for q in self.claims]
        )
        self.np_query_lengths = np.array(
            [self.dict_claim_lengths[q] for q in self.claims]
        )
        self.np_query_char_source = np.array(
            [self.dict_char_left_src[q] for q in self.claims]
        )
        self.query_positions = np.array(
            [self.dict_query_positions[q] for q in self.claims]
        )
        self.np_query_adj = np.array(
            [self.dict_query_adj[q] for q in self.claims]
        )  # query_adj matrix

        # assert self.np_query_lengths.shape == self.np_doc_lengths.shape
        self.padded_doc_length = len(self.dict_doc_contents[self.unique_doc_ids[0]])
        self.padded_doc_char_source_length = len(
            self.dict_char_right_src[self.unique_doc_ids[0]]
        )
        # self.padded_query_length = len(self.np_query_contents[0])
    ...
    def convert_leftright(
        ...
        **kargs
        ...
        for (
            ...
            ids.append(index)
            text_, adj, length_ = self.convert_text(
                row[text_key],
                fixed_length,
                row[length_text_key],
