...
class Dot(nn.Module):
    ...
    def forward(
        self, left: torch.Tensor, right: torch.Tensor, mask: torch.Tensor
...
class ConcatSelfAtt(nn.Module):
    ...
    def forward(
        ...
        linear_out = linear_out.masked_fill(
            doc_mask, -np.inf
        ...
        attended = torch.bmm(
            right.permute(0, 2, 1), attention_weights
        )  # (B, D, L) * (B, L, C) => (B, D, C)
...
class BiLinearTanh(nn.Module):
    ...
    def forward(
        self, left_tsr: torch.Tensor, right_tsr: torch.Tensor, mask: torch.Tensor
    ):
        """
        ...
        linear_out = self.combine(tmp).squeeze(
            -1
        )  # (B, L)  it is equal to v_s^T \tanh(W_1 * h_{ij} + W_2 * a + b_s)
        doc_mask = mask == 0
...
class MultiHeadAttentionSimple(nn.Module):
    def __init__(
        ...
        )  # gom tat ca head vo 1 matrix de nhan co de
        self.w_ks = nn.Linear(d_model, num_heads * d_key)
        self.w_vs = nn.Linear(d_model, num_heads * d_value)
        if init_weights:
            nn.init.normal_(
                self.w_qs.weight, mean=0, std=np.sqrt(2.0 / (d_model + d_key))
            )
            nn.init.normal_(
                self.w_ks.weight, mean=0, std=np.sqrt(2.0 / (d_model + d_key))
            )
            nn.init.normal_(
                self.w_vs.weight, mean=0, std=np.sqrt(2.0 / (d_model + d_value))
            )

        # if attention_type == AttentionType.ConcatNotEqual:
        self.attention_func = ConcatNotEqualSelfAttTransFormer(
            inp_dim=(d_key + d_key), out_dim=d_key
        )
        # else:
    ...
    def forward(
        ...
        len_q = 1
        # transform
        query = self.w_qs(left).view(
            B, len_q, self.num_heads, self.d_key
        )  # (B, 1, num_heads, d_key)
        key = self.w_ks(right).view(
            B, L, self.num_heads, self.d_key
        )  # (B, L, num_heads, d_key)
        value = self.w_vs(right).view(
            B, L, self.num_heads, self.d_value
        )  # (B, L, num_heads, d_value)
        # reshape
        q = (
            query.permute(2, 0, 1, 3).contiguous().view(-1, len_q, self.d_key)
        )  # (num_heads * B) x 1 x dk
        k = (
            key.permute(2, 0, 1, 3).contiguous().view(-1, L, self.d_key)
        )  # (num_heads * B) x L x dk
        v = (
            value.permute(2, 0, 1, 3).contiguous().view(-1, L, self.d_value)
        )  # (num_heads * B) x L x dv
        # compute attention weights
        mask = mask == 0
        mask = mask.unsqueeze(1).repeat(self.num_heads, 1, 1)  # (B * num_heads, 1, L)
        attended, attention_weights = self.attention_func(
        ...
        output = (
            output.permute(1, 2, 0, 3).contiguous().view(B, len_q, -1)
        )  # b x lq x (n*dv)
...
class ConcatNotEqualSelfAttTransFormer(nn.Module):
    ...
    def forward(
        ...
        tmp = torch.tanh(self.linear1(tsr))  # (B, L, out_dim)
        linear_out = self.linear2(tmp)  # (B, L, C)
        doc_mask = mask.squeeze(1).unsqueeze(
            -1
        )  # (B, L) real tokens will be zeros and pad will have non zero (this is for softmax)
        ...
        attention_weights = F.softmax(linear_out, dim=1)  # (B, L, C)
        attended = torch.bmm(
            value.permute(0, 2, 1), attention_weights
        )  # (B, D, L) * (B, L, C) => (B, D, C)
        return attended, attention_weights
