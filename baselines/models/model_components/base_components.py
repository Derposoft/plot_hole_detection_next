...
class GAT(nn.Module):
    def __init__(
        ...
        for i, layer in enumerate(
            self.attentions
        ):  # 神经网络子类需注册:直接定义,add_module,nn.ModuleList
        ...
        self.out_att = nn.ModuleList(
            [
                GraphAttentionLayer(
                    input_size, output_size, dropout=dropout, alpha=alpha, concat=False
                )
                for _ in range(head_num)
            ]
        )
...
class GCN(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, num_layers=1, dropout=0.5):
        ...
        for num in range(self.num_layers):
            Linear_list.append(
                Linear(input_dim, hidden_dim if num == num_layers - 1 else output_dim)
            )
    ...
    def laplacian_normalize(self, adj):
        ...
        d_mat_inv_sqrt = torch.stack(
            [
                d_mat_inv_sqrt[
                    adj.shape[1] * cnt : adj.shape[1] * (cnt + 1),
                    adj.shape[1] * cnt : adj.shape[1] * (cnt + 1),
                ]
                for cnt in range(adj.shape[0])
            ],
            dim=0,
        )
        L_norm = torch.bmm(
            torch.bmm(d_mat_inv_sqrt, adj.type_as(d_mat_inv_sqrt)), d_mat_inv_sqrt
        )
        return L_norm
