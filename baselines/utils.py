...
def create_mask_tensor_image(
    left_indices: torch.Tensor, right_indices: torch.Tensor, threshold: int = 0
):
    """
    Creating masking of two tensor. These two tensors are integer tensor
    Parameters
    ...
    if B1 == 1:
        left_mask = left_mask.expand(B, M1, 1)  # during testing
...
def get_sorted_index_and_reverse_index(base_array: np.ndarray):
    ...
    restoring_indices = r[
        :, 1
    ]  # the retoring indices. This method is tested very carefully.
    return new_indices, restoring_indices


def packing_sequence(
    seq: torch.Tensor, seq_lens: np.ndarray, new_index
) -> torch.Tensor:
    """
    ...
    return rnn_utils.pack_padded_sequence(
        seq[new_index], seq_lens[new_index], batch_first=True
    )
...
def idf(total_docs: int, term_freq: int) -> float:
    ...
    """
    assert (
        term_freq <= total_docs
    ), "The number of documents that contain a term must be smaller than total_docs"
...
def cosine_distance(a: torch.Tensor, b: torch.Tensor):
    ...
    return torch.sqrt(
        torch.abs(A_square.unsqueeze(-1) - 2 * dot + B_square.unsqueeze(1)) + 1e-10
    )
...
def _get_doc_context_copacrr(
    doc: torch.Tensor, doc_mask: torch.Tensor, context_window_size: int
) -> torch.Tensor:
    """
    ...
    right = (
        context_window_size - left - 1
    )  # in case context windows is an even number then left=x//2, right=x-x//2
...
def auto_rnn(
    ...
    """
    return rnn_cell(
        (input_feats, lens, new_indices, restoring_indices),
        max_len=max_len,
        return_h=False,
    )[0]
