...
class BiDAF(BaseModel):
    ...
    def __init__(self, params):
        ...
        for i in range(2):
            setattr(
                ...
                "highway_linear%s" % i,
                nn.Sequential(
                    Linear(params["word_dim"], params["word_dim"]), nn.ReLU()
                ),
            )
            setattr(
                self,
                "highway_gate%s" % i,
                nn.Sequential(
                    Linear(params["word_dim"], params["word_dim"]), nn.Sigmoid()
                ),
            )

        # 3. Contextual Embedding Layer
        self.context_LSTM = LSTM(
            input_size=params["word_dim"],
        ...
        self.modeling_LSTM1 = LSTM(
            input_size=params["hidden_size"] * 8,
            hidden_size=params["hidden_size"],
            bidirectional=True,
            batch_first=True,
            dropout=params["dropout"],
    ...
    def predict(
        ...
        queries_lens, docs_lens = (
            kargs[KeyWordSettings.Query_lens],
            kargs[KeyWordSettings.Doc_lens],
        )
