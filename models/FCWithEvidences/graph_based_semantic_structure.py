...
class Graph_basedSemantiStructure(BasicFCModel):
    ...
    def __init__(self, params):
        ...
        evd_input_size += (
            dim * self.num_att_heads_for_words * self.num_att_heads_for_evds
        )  # twice times for two times attention
        if self.use_article_source:
    ...
    def _evidence_level_attention_new(
        self,
        left_tsr: torch.Tensor,
        right_tsr: torch.Tensor,
        ...
        **kargs
        ...
        mask = (
            torch.sum(full_padded_document, dim=-1) >= 1
        ).float()  # (B, n), 0 is for padding that why >= 1
        if self.use_article_source:
    ...
    def _get_word_attention_func(self, dim: int):
        ...
        self.self_att_word = ConcatNotEqualSelfAtt(
            inp_dim=input_dim, out_dim=dim, num_heads=self.num_att_heads_for_words
        )
        # else:
    ...
    def _get_evd_attention_func(self, dim: int):
        ...
        if self.use_claim_source:
            input_dim += self.claim_emb_size
        if self.use_article_source:
            input_dim += self.article_emb_size
        self.self_att_evd = ConcatNotEqualSelfAtt(
            inp_dim=input_dim, out_dim=dim, num_heads=self.num_att_heads_for_evds
        )
        # else:
    ...
    def predict(
        self, query: torch.Tensor, doc: torch.Tensor, verbose: bool = False, **kargs
    ) -> np.ndarray:
